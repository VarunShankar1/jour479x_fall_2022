...
```{r}
html <- "https://www.aicyellowjackets.com/sports/wbkb/2022-23/roster?view=list" %>% 
  read_html() %>% 
  html_table()

americaninternational <- html[[1]]
```
American international is a difficult scrape, lower on priority list
```{r}
html <- "https://assumptiongreyhounds.com/sports/womens-basketball/roster" %>% 
  read_html() %>% 
  html_table()

assumption_roster <- html[[3]] 

assumption_roster <- assumption_roster %>% 
  mutate(HighSchool = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), tail, 1)) %>% 
  mutate(Hometown = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), head, 1))
```
```{r}
html <- "https://bsubeavers.com/sports/womens-basketball/roster?view=2" %>% 
  read_html() %>% 
  html_table()

bemidji_state_roster <- html[[3]] 
```
```{r}
html <- "https://www.bethanybison.com/sports/wbkb/2022-23/roster?view=list" %>% 
  read_html() %>% 
  html_table()

bethany_college_roster <- html[[1]] 
```
Bethany College is a difficult scrape, lower on priority list
Brandeis doesn't have 22-23 roster listed
```{r}
html <- "https://bridgewatereagles.com/sports/womens-basketball/roster?view=2" %>% 
  read_html() %>% 
  html_table()

bridgewater_roster <- html[[3]] 
bridgewater_roster <- bridgewater_roster %>% 
  mutate(HighSchool = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), tail, 1)) %>% 
  mutate(Hometown = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), head, 1))
```
```{r}
html <- "https://gocaltech.com/sports/womens-basketball/roster?view=2" %>% 
  read_html() %>% 
  html_table()

caltech_roster <- html[[3]] 
caltech_roster <- caltech_roster %>% 
  mutate(HighSchool = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), tail, 1)) %>% 
  mutate(Hometown = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), head, 1)) 
```
```{r}
html <- "https://calvinknights.com/sports/womens-basketball/roster?view=2" %>% 
  read_html() %>% 
  html_table()

calvin_roster <- html[[3]] 
calvin_roster <- calvin_roster %>% 
  mutate(HighSchool = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), tail, 1)) %>% 
  mutate(Hometown = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), head, 1)) 
```
```{r}
html <- "https://dickinsonathletics.com/sports/wbball/roster?view=2" %>% 
  read_html() %>% 
  html_table()

dickinson_roster <- html[[3]] 
dickinson_roster <- dickinson_roster %>% 
  mutate(HighSchool = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), tail, 1)) %>% 
  mutate(Hometown = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), head, 1)) 
```
```{r}
...
distcolumbia_roster <- html[[3]] 
distcolumbia_roster <- distcolumbia_roster %>% 
  mutate(HighSchool = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), tail, 1)) %>% 
  mutate(Hometown = sapply(strsplit(`Hometown / High School`, " / ", fixed=TRUE), head, 1)) 
```
```{r}
html <- "https://gustavus.edu/athletics/wbb/roster/" %>% 
  read_html() %>% 
  html_table()

gustavus_adolphus_roster <- html[[1]] 

gustavus_adolphus_roster <- gustavus_adolphus_roster %>% 
  clean_names() %>% 
